 ...
 * Return: 0 if the first n bytes of string1 and string2 are equal, otherwise non-zero
 */
...
/**
 * readfile - read from a file and print an error message upon failure
 * @file: the file descriptor to read from
 * @buffer: the buffer to write to
 * @count: the number of bytes to read
 */
void readfile(int file, char *buffer, size_t count)
{
	if (read(file, buf, count) != -1)
		return;
...
int elf_data(const unsigned char *buffer)
{
	...
	if (buffer[EI_DATA] == ELfileATA2MSB)
	{
		printf("2's complement, big endian\n");
		return (1);
	}
	if (buffer[EI_DATA] == ELfileATA2LSB)
	{
...
int main(int argc, const char *argv[])
{
	...
	readfile(file, (char *) buffer, bit_mode / 8);
